...
4. Write sibling culture files (`AppResources.es.resx` next to the English source). Create the file if it is missing. Do not rename keys. Do not add keys that are not in the worklist. Do not overwrite `current` keys.
